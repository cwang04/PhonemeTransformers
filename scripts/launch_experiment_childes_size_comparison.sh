...
do
    ...
    do
        echo "Launching jobs for data size: $data_size and model: $model"
        sbatch launch_slurm.wilkes3 experiment=childes_comparison_01 model=$model data_preprocessing.subsample=$data_size experiment.name=$model-$data_size-01 $@
        sbatch launch_slurm.wilkes3 experiment=childes_comparison_03 model=$model data_preprocessing.subsample=$data_size experiment.name=$model-$data_size-03 $@
        sbatch launch_slurm.wilkes3 experiment=childes_comparison_05 model=$model data_preprocessing.subsample=$data_size experiment.name=$model-$data_size-05 $@
    done
done
